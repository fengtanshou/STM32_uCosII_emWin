	...
	do {
		...
		switch (Key) {
			...
			case GUI_ID_CANCEL:
				break;
			default:
				EDIT_AddKey(hEdit, Key);	/* Ö¸¶¨×Ö·û±»Ìí¼Óµ½EDITÐ¡¹¤¾ßµÄÓÃ»§ÊäÈëÖÐ */
		}
	} while ((Key != 'N') && (Key!=GUI_ID_CANCEL) && (Key!=0));
	/* Cleanup */
	for (i=0; i< countof(ahButton); i++) {
...
void GUIDEMO_Touch(void) {
  ...
  do {
	...
	ahButton[3] =  BUTTON_Create( XMid + 10, YMid + 70, 80, 30, ID_MY,  BUTTON_CF_SHOW );
    ...
    if (r==0) {
      r = ID_KEYBOARD;
      BUTTON_SetState(ahButton[1],BUTTON_STATE_PRESSED);	/* ½«Ö¸¶¨°´Å¥ÉèÖÃÎª°´ÏÂ×´Ì¬ */
      GUIDEMO_Delay(500);
    }
    for (i=0; i< countof(ahButton); i++) {
      BUTTON_Delete(ahButton[i]);
    ...
    switch (r) {
		...
		case ID_MY:
				MESSAGEBOX_Create("Application will be closed...", "Message", GUI_MESSAGEBOX_CF_MODAL);
			break;
    }
  } while (r && (r!='n') && (r!='N'));
}
