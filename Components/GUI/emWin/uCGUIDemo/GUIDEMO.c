...
extern void ScreenSineWave(void);
extern void Menu_Configuration(void);
...
static void _UpdateCmdWin(void) {
    ...
    WM_InvalidateWindow(_ahInfoWin[1]);		/* 使窗口无效 */
  #endif
}
...
//     GUI_CURSOR_Show();
  ...
  GUI_Clear();
    ...
    _ahFrameWin[1] = FRAMEWIN_Create("Control", NULL, WM_CF_SHOW| WM_CF_STAYONTOP, 
											LCD_GetXSize() - 80, LCD_GetYSize() - 60, 80, 60);	/* 创建一个FRAMEWIN 小工具。（弃用） */
    _ahInfoWin[1] = WM_CreateWindowAsChild( 0, 0, 0, 0, WM_GetFirstChild(_ahFrameWin[1]), 
											WM_CF_SHOW| WM_CF_STAYONTOP, &_cbCmdWin, 0);		/* 创建子窗口。 */
    _ahButton[0] = BUTTON_CreateAsChild( 4, 20, _ButtonSizeX, _ButtonSizeY, _ahInfoWin[1], 'S' , 
											WM_CF_SHOW | WM_CF_STAYONTOP | WM_CF_MEMDEV);		/* 将BUTTON 小工具创建为子窗口。（弃用） */
    _ahButton[1] = BUTTON_CreateAsChild(40, 20, _ButtonSizeX, _ButtonSizeY, _ahInfoWin[1], 'N' , 
											WM_CF_SHOW | WM_CF_STAYONTOP | WM_CF_MEMDEV);
    BUTTON_SetText(_ahButton[0], "Stop");
    BUTTON_SetText(_ahButton[1], "Next");
    _UpdateCmdWin();						/* 函数作用是，使窗口无效。就会顺利进入回调函数中 */
    WM_ExecIdle();							/* 通过执行回调重绘无效窗口（所有工作）。 */
  #endif
...
//     _ahFrameWin[0] = FRAMEWIN_Create("礐/GUI Demo", NULL, WM_CF_STAYONTOP, xPos, yPos, xSize, ySize);
...
//   #endif
	...
	for (_iTest = 0; _apfTest[_iTest]; _iTest++) {
		...
		GUI_SaveContext(&ContextOld);		/* 保存GUI 环境。 */
		_iTestMinor = 0;
		_UpdateCmdWin();					/* 函数作用是，使窗口无效。就会顺利进入回调函数中 */
		(*_apfTest[_iTest])();				/* 执行当前界面函数 */
		_CmdNext = 0;
		GUI_RestoreContext(&ContextOld);	/* 恢复GUI 环境。 */
	}
