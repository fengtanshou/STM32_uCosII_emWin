...
static const GUI_ConstString _asLang[] = {
  ...
  "Deutsch", 
  "Fran鏰is", 
  "Espa\xf1ol", /* "Espa駉l" -> "Espa\xf1ol" to avoid problems with NC308 */
  "Italiano", 
  NULL 
};
...
void GUIDEMO_DemoFrameWin(void) {
  ...
  WM_SetFocus(hListBox);			/* 将输入焦点设置到指定窗口 */
  LISTBOX_SetFont(hListBox, pFont);
  GUIDEMO_Wait();
  for (i = 0; (i < 10) && !GUIDEMO_CheckCancel(); i++) {
    LISTBOX_IncSel(hListBox);			/* 增加列表框选定范围（将指定列表框的选择栏下移一个项目） */
    GUIDEMO_Delay(250);
	}
  for (i = 0; (i < 10) && !GUIDEMO_CheckCancel(); i++) {
    LISTBOX_DecSel(hListBox);			/* 递减列表框选取（将指定列表框的选择条向上移动一个项目） */
    GUIDEMO_Delay(250);
	}
  GUIDEMO_Delay(500);
  LISTBOX_Delete(hListBox);	/* 删除列表框 */
  FRAMEWIN_Delete(hFrame);	/* 删除窗口 */
